...
class CSV(ParserDefinition):
    ...
    _cache: dict[str, int | None | list[str]] = pydantic.PrivateAttr({"log_index": 0, "headers": None})
    ...
    def _parse_log(
        ...
        if (
            self.header_line_index and
            self._cache["log_index"] + len(_lines) < self.header_line_index
        ):
            return {}, {}
        
        if (
            self.header_line_index and
            self._cache["log_index"] < self.header_line_index
        ):
            _lines = _lines[self.header_line_index:]
        ...
        if (_headers := self.headers or self._cache["headers"]):
            ...
            _out = [
                row for row in _csv_reader
            ]

        if self.tracked_values:
            _out = [
                {k: v for k, v in row.items() if any(re.match(tracked, k) for tracked in self.tracked_values)}
                for row in _out
            ]
