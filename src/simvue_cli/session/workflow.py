...
from simvue_cli.session.config import SessionConfiguration, Step, Status, Simulation, TrackedFile
...
class Workflow(pydantic.BaseModel):
    ...
    offline: bool = False
    _session_id: str | None = pydantic.PrivateAttr(datetime.datetime.now(datetime.UTC).strftime("%d_%m_%y_%H_%M_%S_%f"))

    @classmethod
    @pydantic.validate_call
    def from_file(cls, session_file: pydantic.FilePath, offline: bool=False) -> typing.Self:
    ...
    def _execute(self, step: Step) -> Step:
        if step.status == Status.Waiting and step.inputs:
            raise RuntimeError(f"Failed to find required inputs for next pending step '{step.label}'")
        if step.status == Status.Completed:
            return step
        if isinstance(step, Simulation):
            _init_args: dict[str, str | int | dict | bool] = (
                self.config.options.model_dump(warnings='error') |
